...
void show_tile(int bank, int tile_n, int row, int col, int shift_x, int shift_y) {
	...
	uint16_t tile_address = bank + tile_n * 0x10;
	...
	if (nametable_id == 0) {
 	...
 	for (int row = 0; row < 30; row++) {
		for (int col = 0; col < 32; col++) {
			uint16_t tile_n = VRAM[nametable_address + ( row * 32) + col];
			show_tile(bank_address[bg_bank], tile_n, row, col, shift_x, shift_y);
		}
	}
}
...
unsigned char fetch_nt_entry() {
	unsigned char entry = VRAM[v];
	v++;
	return entry;
}
...
void ppu_clock(int cpu_cycles) {
	...
	// 			break;
	// 		}
	// 		show_tile(0, tile_n, row, col, 0, 0);
	// 		tile_n++;
	// 	}
	// }
	...
	for (int n = 0; n < 0x400; n++) {
		unsigned char nt_entry = fetch_nt_entry();
		unsigned char tile_n = VRAM[0x2000 + n];
		col = n % 32;
		row = n / 32;
	...
	uint16_t tile_address = 0x2000 + tile_n * 0x10;
